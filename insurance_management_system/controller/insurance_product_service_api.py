...
from insurance_management_system.helper.insurance_management_helper_class import get_insurance_product_list
from insurance_management_system.helper.model_class import InsuranceProductRequest

class InsuranceProductServiceAPIView(APIView):
    ...
    def get(self, request):
        try:
            record = build_request_with_user(InsuranceProductRequest, request, method='GET')
            #record = AssetInfoRequest(**request.data)
            result = get_insurance_product_list(record)
